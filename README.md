...
它在用户连接的本地 Chrome 中工作。未知站点首遍只用截图和视觉操作连续走通完整目录、分页、详情、画廊与字段；走通后从入口正向重放，第二遍才把已验证路径映射为可复用的 DOM、CDP、Network 与高清图片规则。后续运行优先回放通过代表详情验证的站点 Profile；一旦模板或覆盖异常，模型会重新观察并修补规则，而不是继续输出 partial 数据。
