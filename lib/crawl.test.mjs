...
describe("extractProductsBatch", () => {
  it("keeps complete records and routes incomplete ones to the upgrade path", async () => {
    const pages = {
...
describe("upgradeProducts", () => {
  ...
  it("stops opening pages once the interactive path stops helping", async () => {
    // Every page is missing price, so the upgrade never improves anything. The
    // budget must cut it off instead of walking the entire remaining catalog.
    ...
    expect(tab.visited.length).toBe(3);
    expect(result.failed.some((entry) => entry.reason === "upgrade_disabled")).toBe(true);
    // The partial records are still kept — the worker keeps these too.
