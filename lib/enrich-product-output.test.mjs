...
describe("Supply Smart enrich product output", () => {
  it("maps a crawl record to the API input and keeps Facts images", () => {
    ...
    const { healthFunctions, ...rest } = input;
    expect(rest).toEqual({
      domain: "shaklee.com",
    ...
    // health functions are now aligned to the controlled vocab as {id,name}
    expect(healthFunctions.map((h) => h.name)).toEqual(["Cellular Health", "Anti-Aging Support"]);
    expect(healthFunctions.every((h) => typeof h.id === "string")).toBe(true);
  ...
  it("matches every current enrich request field and preserves the full product URL", () => {
    ...
    expect(input.healthFunctions.map((h) => h.name)).toEqual(["Joint Health"]);
    const { healthFunctions: _hf, ...restJson } = input;
    expect({ json: restJson }).toEqual({
      json: {
        domain: "shaklee.com",
  ...
  it("wraps each request in the oRPC json envelope and reports invalid records", () => {
    ...
    expect(row.productGroupId).toBe("https://shop.test/a");
    expect(row.healthFunctions.map((h) => h.name)).toEqual(["Immune Support"]);
    const { healthFunctions: _hf, productGroupId: _g, ...rowRest } = row;
...
describe("variant expansion: one enrich input per SKU", () => {
  ...
  it("emits one input per variant, each a distinct SKU sharing productGroupId", () => {
    const inputs = toEnrichProductInputs(variantRecord());
    expect(inputs).toHaveLength(2);

    // all variants aggregate under the same parent key
    const groups = new Set(inputs.map((i) => i.productGroupId));
    expect(groups).toEqual(new Set(["https://shop.test/products/collagen"]));

    // each row carries its own sku / price / variant URL / options
    expect(inputs[0]).toMatchObject({
      sku: "PFF-01199", price: "29.90", variantId: "111",
      productUrl: "https://shop.test/products/collagen?variant=111",
      variantOptions: { Saveur: "Neutre", Quantité: "1 BOITE" },
    });
    expect(inputs[1]).toMatchObject({ sku: "PFF-01542", price: "32.50", variantId: "222" });

    // base fields are inherited by every variant row
    expect(inputs[0].productName).toBe("Collagen");
    expect(inputs[1].productName).toBe("Collagen");
  ...
  it("a variant-less product yields one row keyed to its own parent URL", () => {
    const inputs = toEnrichProductInputs(apiReadyRecord("Solo"));
    expect(inputs).toHaveLength(1);
    expect(inputs[0].productGroupId).toBe("https://www.example.com/products/solo");
    expect(inputs[0]).not.toHaveProperty("variantId");
  });
  ...
  it("export expands variants into multiple submittable rows", () => {
    ...
    expect(exported.inputs.map((i) => i.sku)).toEqual(["PFF-01199", "PFF-01542"]);
  });
});
