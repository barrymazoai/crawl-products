...
export async function verifyRunArtifacts(outDir, options = {}) {
  ...
  const queueEntries = Array.isArray(semanticQueue.value) ? semanticQueue.value : [];
  for (const entry of queueEntries) {
    const status = entry?.status;
    const terminal = ["enriched", "review", "needs_browser"].includes(status);
    check(
    ...
    if (status === "review" || status === "needs_browser") {
      check(
