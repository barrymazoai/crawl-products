...
function applyDetailExtractionProfile(input) {
	...
	// A mapped title is authoritative. Do not retain a deterministic fallback
	// name (for example a site-wide promotion H1) beside the real product title.
	if (stringField(fields.title)) fields.name = fields.title;
...
function firstSelectorText(selector, html) {
	...
	// Visual replay commonly produces a compound simple selector such as
	// `h2.product-title`, `.product-title.desktop`, or `#facts.panel`.
	// The old parser only understood one tag/class/id token, so a valid mapped
	// selector silently fell through to deterministic fallback (often a promo
	// heading). Support a bounded tag/id + class-list form without pretending
	// to implement arbitrary CSS.
	const compound = trimmed.match(/^([a-z][\w:-]*)?(?:#((?:\\.|[\w:-])+))?((?:\.(?:\\.|[\w:-])+)+)$/i);
	if (compound) {
		const tag = compound[1] || "[^>]+";
		const id = compound[2]
			? `(?=[^>]*\\bid=["']${escapeRegex(compound[2].replace(/\\([.#:[\\]])/g, "$1") )}["'][^>]*>)`
			: "";
		const classes = [...compound[3].matchAll(/\.((?:\\.|[\w:-])+)/g)]
			.map((match) => match[1].replace(/\\([.#:[\\]])/g, "$1"));
		const classLookaheads = classes
			.map((className) => `(?=[^>]*\\bclass=["'][^"']*\\b${escapeRegex(className)}\\b[^"']*["'])`)
			.join("");
		const opening = `<${tag}\\b${id}${classLookaheads}[^>]*>`;
		return firstTextFromHtml(new RegExp(`${opening}([\\s\\S]*?)<\\/[^>]+>`, "i"), html);
	}
	if (/^[a-z][\w:-]*$/i.test(trimmed)) return firstTextFromHtml(new RegExp(`<${escapeRegex(trimmed)}\\b[^>]*>([\\s\\S]*?)<\\/${escapeRegex(trimmed)}>`, "i"), html);
	if (trimmed.startsWith(".")) {
		const className = escapeRegex(trimmed.slice(1).replace(/\\([.#:[\\]])/g, "$1"));
		return firstTextFromHtml(new RegExp(`<[^>]+class=["'][^"']*\\b${className}\\b[^"']*["'][^>]*>([\\s\\S]*?)<\\/[^>]+>`, "i"), html);
	}
	if (trimmed.startsWith("#")) {
		const id = escapeRegex(trimmed.slice(1).replace(/\\([.#:[\\]])/g, "$1"));
...
const OPTIONAL_DETAIL_FIELDS = new Set(["sku"]);
function splitMissingDetailExtractionFieldsByPolicy(missingFields, profile) {
	const ignoredMissingFields = missingFields.filter((field) => {
		const missingBehavior = profile?.fieldPolicy?.[field]?.missingBehavior;
		if (missingBehavior === "allow_missing") return true;
		return OPTIONAL_DETAIL_FIELDS.has(field) && missingBehavior !== "require_fallback";
	});
	return {
