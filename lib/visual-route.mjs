...
export async function mapVisualRouteAction(tab, action = {}, opts = {}) {
  ...
  return tab.playwright.evaluate(({ actionText, targetUrl }) => {
    ...
    const stableElementSelectors = [...new Set(generalCandidates)];
    if (stableElementSelectors.length === 0) stableElementSelectors.push(tag);
...
export function extractionProfilesFromVisualRoute(routeInput) {
  ...
  for (const checkpoint of route.fieldJourney.fields) {
    const { field, availability, targetSelector, revealAction } = checkpoint;
    ...
    const interactionHint = revealAction ? {
      field,
...
export async function replayVisualRoute(tab, routeInput, opts = {}) {
  ...
  for (let index = 0; index < enrichedSteps.length - 1; index += 1) {
    ...
    if (mapping) {
      const reusableGeneralSelector = step.pageRole !== "listing"
  ...
  for (const fieldItem of route.fieldJourney.fields) {
    ...
    const targetMapping = await runOperation(
    ...
    if (!targetMapping?.selector || !targetMapping?.quality?.valid) {
  ...
  const categorySelectors = checkpoints
    .filter((checkpoint) =>
      ["home", "portfolio"].includes(checkpoint.actionFromPageRole)
