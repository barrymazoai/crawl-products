...
export const DEFAULT_HARVEST_BUDGETS = Object.freeze({
  ...
  wallClockMinutes: 60,
  stallMinutes: 5,
  // Hard ceiling on any single extract/upgrade call. The wall-clock watchdog
  // only checks between products, so a browser call that hangs internally
  // would otherwise stall forever; this bounds each call so a wedged site
  // always aborts resumable instead of freezing the whole wave.
  ...
  return {
    ...
    termination: {
      ...
      budgets: {
        ...
        operationTimeoutMinutes: positiveNumber(
          budgets.operationTimeoutMinutes,
          DEFAULT_HARVEST_BUDGETS.operationTimeoutMinutes,
        ),
