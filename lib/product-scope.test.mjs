...
describe("nutrition-only product scope", () => {
  ...
  it("does not reject a variant URL whose query names a package size", () => {
    expect(classifyNutritionProductUrl(
      "https://example.com/products/electrolyte?selected_pack_size=30-capsules&variant=42",
    )).toMatchObject({
      included: true,
  ...
  it("keeps a single product pack-size variant when the selected state is explicit", () => {
    expect(classifyNutritionSingleProduct(record("Electrolyte Pack", {
      fields: {
        categories: ["Nutrition"],
        form: "powder drink mix",
        variant_options: [{ name: "Pack Size", value: "30 capsules" }],
        facts_images: [{ type: "Supplement Facts", image_url: "https://cdn.example/facts.png" }],
      },
    }))).toMatchObject({
      included: true,
