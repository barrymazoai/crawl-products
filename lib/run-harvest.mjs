...
import { filterNutritionSingleProductRecords } from "./product-scope.mjs";
...
export async function runHarvest(browser, tab, planInput, opts = {}) {
  ...
  const hooks = {
    ...
    extract: opts.hooks?.extract
      || ((urls, extractOpts) => extractProductsBatch(browser, urls, extractOpts)),
    ...
    filterScope: opts.hooks?.filterScope || filterNutritionSingleProductRecords,
  };
  ...
  while (index < queue.length) {
    ...
    // Anything in the chunk with no terminal state after both methods failed.
    for (const url of chunk) {
      if (processed.has(url)) continue;
      const attempts = attemptsByUrl.get(url) || [];
      const methodsTried = new Set(attempts.map((a) => a.method));
      if (methodsTried.has("batch_content") && methodsTried.has("rendered_upgrade")) {
        processed.set(url, "failed");
      } else if (!needsUpgrade.includes(url)) {
        // batch produced neither a record nor an upgrade request — treat as
        // failed with the batch as the recorded attempt.
