...
const BINDING_LOST_PATTERN =
  /browser is not available|browser_unavailable|binding (?:lost|closed)|native pipe is closed|extension disconnected/i;
...
export async function runHarvest(browser, tab, planInput, opts = {}) {
  ...
  while (index < queue.length) {
    ...
    let batch;
    try {
      batch = await hooks.extract(chunk, {
        fields: opts.fields,
    ...
    if (needsUpgrade.length > 0) {
      let upgraded;
      try {
        upgraded = await hooks.upgrade(needsUpgrade, {
          fields: opts.fields,
          profile: plan.route.detailProfile,
          imageProfile: plan.route.imageProfile || undefined,
