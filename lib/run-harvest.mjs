...
/**
 * Bound a single hook call by wall-clock time. If the underlying browser
 * operation hangs internally (a goto that never returns, a dead binding that
 * never rejects), the between-products watchdog can never fire — this races a
 * real timer so the call always settles. A timeout is surfaced as a
 * binding-lost-class error, which the extract/upgrade paths already turn into
 * a resumable incomplete abort.
 */
function withOperationTimeout(promise, ms, label) {
  if (!ms || ms <= 0) return Promise.resolve(promise);
  return new Promise((resolve, reject) => {
    const timer = setTimeout(() => {
      reject(Object.assign(new Error(`operation_timeout:${label}`), {
...
export async function runHarvest(browser, tab, planInput, opts = {}) {
  ...
  const operationTimeoutMs = opts.operationTimeoutMs
    ?? budgets.operationTimeoutMinutes * 60_000;
  ...
  while (index < queue.length) {
    ...
    let batch;
    try {
      batch = await withOperationTimeout(
        hooks.extract(chunk, {
          fields: opts.fields,
    ...
    if (needsUpgrade.length > 0) {
      let upgraded;
      try {
        upgraded = await withOperationTimeout(
