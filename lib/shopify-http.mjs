 ...
 * sampleCount } on success (positive signal only) or null. The caller decides
 * to route through the HTTP channel vs the browser.
 */
export async function probeShopifyCatalog(input, opts = {}) {
  const base = origin(input);
  if (!base) return null;
  const body = await fetchJson(`${base}/products.json?limit=1`, opts.timeoutMs);
  if (!body || !Array.isArray(body.products)) return null;
  return { origin: base, available: true };
}
