 ...
 *   fast  — `browser.tabs.content({urls})` loads a whole batch in background
 *           tabs and returns HTML. No interaction, so it only works when the
 *           template exposes the requested fields in the loaded document.
 ...
 *
 * Unknown-site entry discovery is intentionally not implemented here. Batch
 * crawling starts only from a mapped screenshot-first route or a valid saved
 * profile. Every field value still comes from a rendered product page.
 */
...
export async function collectRenderedInlineProductRecords(tab, baseUrl, opts = {}) {
  ...
  for (const candidate of candidates) {
    ...
    if (images.length === 0) continue;
    const sourceUrl = `${String(baseUrl).replace(/#.*$/, "")}#inline-product=${encodeURIComponent(titleKey)}`;
 ...
 *
 * Stops on: reaching maxItems, running out of next pages, or two consecutive
 * pages that add nothing new (the worker's `no_new_products` rule, which
 * catches both genuine exhaustion and pagination that silently loops).
 */
export async function collectProductUrls(tab, seedUrls, opts = {}) {
  ...
  for (const seed of seedUrls) {
    ...
    for (let page = 0;
      ...
      let semanticUrls = await collectRenderedProductCardUrls(tab, listing.url, opts);
      if (opts.scrollListings !== false) {
        ...
        semanticUrls = dedupe([
          ...semanticUrls,
          ...await collectRenderedProductCardUrls(tab, listing.url, opts),
        ]);
      }
      ...
      if (added === 0) {
        ...
        for (const record of inline) inlineRecords.set(record.sourceUrl, record);
      ...
      pageUrl = findNextListingPage(listing.url, body, visited);
    }
    if (found.size + inlineRecords.size >= maxItems) break;
  }
...
function extractOne(url, html, profile, requestedFields, imageProfile) {
  ...
  const result = applyDetailExtractionProfile({
    profile: profile || { version: 1, kind: "detail-extraction" },
    evidence: detailEvidence(url, html, requestedFields, imageProfile),
  ...
  return { record: result.record, missing, requiredMissing: split.requiredMissingFields };
}
...
export async function extractProductsBatch(browser, urls, opts = {}) {
  ...
  for (let start = 0; start < urls.length; start += size) {
    ...
    for (const [index, result] of results.entries()) {
      ...
      const { record, requiredMissing } = extractOne(url, result.content, profile, requestedFields, imageProfile);
      if (record && requiredMissing.length === 0) {
        records.push(record);
        if (recordNeedsRenderedImageUpgrade(record, requestedFields, opts)) {
  ...
  return { records, needsUpgrade: dedupe(needsUpgrade), failed };
}
 ...
 *
 * `disableAfter` mirrors the worker's upgrade-disable rule: if the interactive
 * path keeps failing to improve on the batch result, the field is missing from
 * the whole template, not from these particular pages, and continuing to open
 * pages one by one just burns time.
 */
export async function upgradeProducts(tab, urls, opts = {}) {
  ...
  const profile = opts.profile || null;
  const imageProfile = opts.imageProfile || null;
  const disableAfter = opts.disableAfter ?? 3;
  ...
  for (const [index, url] of urls.entries()) {
    if (disableAfter > 0 && fruitless >= disableAfter) {
      log("upgrade_disabled", { after: index, reason: "no_improvement" });
      for (const remaining of urls.slice(index)) {
        const entry = { url: remaining, reason: "upgrade_disabled" };
        if (baselineByUrl.has(remaining)) skipped.push(entry);
    ...
    try {
      ...
      await dismissOverlays(tab, { log });
      if (profile) await expandDetailSections(tab, profile, { log });
      if (wantsImages) await lazyLoadScroll(tab);
      ...
      if (record) {
        ...
        // Accept a record that is merely incomplete — the worker keeps these
        // too, with the gaps recorded — but count it against the budget.
        fruitless = requiredMissing.length > 0 || imageUpgradeWasNeeded && !imageImproved
          ? fruitless + 1
          : 0;
      } else {
        const entry = { url, reason: "no_record" };
        if (baselineByUrl.has(url)) skipped.push(entry);
        else failed.push(entry);
        fruitless += 1;
      }
    } catch (error) {
...
function listingProfileFromMappedVisualRoute(routeInput) {
  ...
  for (const step of route.steps) {
    ...
    if (["home", "portfolio"].includes(step.pageRole)) {
      if (step.action?.generalSelector) categoryLinkSelectors.push(selector);
    } else if (step.pageRole === "listing") {
...
export async function crawlSite(browser, tab, startUrl, opts = {}) {
  ...
  const candidateMaxItems = nutritionSinglesOnly
    ? Math.min(1_000, Math.max(maxItems + 20, Math.ceil(maxItems * 1.5)))
    : maxItems;
  ...
  const routeValidation = validateMappedVisualRoute(runtimeVisualRoute, { fields });
  if (!routeValidation.valid) {
    const missing = routeValidation.missingFields.length > 0
  ...
  if ((runtimeListingProfile?.categoryLinkSelectors || []).length > 0) {
  ...
  if (productUrls.length + inlineRecords.length < candidateMaxItems
      && discovery.listingSeeds.length > 0) {
  ...
  const originalImages = fields.some((field) => field === "images" || field === "image")
    ...
    ? await upgradeProductImageUrls(tab, records, { ...runtimeOpts, log })
    : null;

  let savedProfilePath = null;
  if (opts.profileDir && opts.saveProfile !== false) {
  ...
  return {
    ...
    stats: {
      ...
      failed: [...batch.failed, ...upgrade.failed].length,
    ...
    failures: [...batch.failed, ...upgrade.failed].slice(0, 20),
...
export async function crawlPortfolio(browser, tab, startUrl, opts = {}) {
  ...
  return {
    records: mergeProductRecords(records),
    stats: {
      startUrl,
