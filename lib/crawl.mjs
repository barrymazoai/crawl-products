 ...
 *
 * `disableAfter` is scoped to a baseline missing-field signature. A circuit
 * breaker for one template/field shape must not stop a different product
 * family, and URLs without any usable batch baseline are always attempted.
 */
export async function upgradeProducts(tab, urls, opts = {}) {
  ...
  for (const [index, url] of urls.entries()) {
    ...
    if (signature && disableAfter > 0
      ...
      skipped.push({ url, reason: "upgrade_disabled", signature });
      continue;
    }
    try {
...
export async function crawlSite(browser, tab, startUrl, opts = {}) {
  ...
  const unresolvedFailures = [...batch.failed, ...upgrade.failed]
    .filter((entry) => !extractedUrls.has(entry.url));
